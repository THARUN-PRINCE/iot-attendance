...
st.markdown(
    ...
    <style>
        ...
        /* General Input styling for Name */
        .stTextInput input {
            background-color: #ffffff !important;
            color: #111111 !important;
...
if st.session_state.is_registered:
  ...
  # Fully responsive 4 separate boxes HTML/JS Component
  otp_input_html = """
    <!DOCTYPE html>
    <html>
    <head>
  ...
  with st.form("ref_reg_form"):
    ...
    if submit_btn:
      if len(captured_otp) == 4 and student_name:
        try:
          # 1. Push data to Firebase Realtime Database
          ref = db.reference("registrations")
          ref.push({"otp": captured_otp, "student_name": student_name})

          # 2. Send data to Google Sheet via Apps Script Web App
